...
2.Azure AD Authentication Library (ADAL) のメソッドを呼び出すことにより、ユーザーを認証してアクセス トークンを取得する
...
- [ユーザーを認証してアプリの承認を得る](#msg_get_app_authorized)
...
この要求は初期トークン取得要求とほぼ同じであることに注意してください。要求ペイロード内に 2 つの違いがあります。具体的には、`grant_type` パラメーターの値が (`code` ではなく) `refresh_token` になっています。
