...
###REST により Microsoft Graph API サービスを呼び出す
Microsoft Graph API リソースにアクセスして操作するには、リソースで許可される次の操作のいずれを使用して、リソース URL を呼び出し指定します。   
...
- `[odata_query_parameters]` は、OData クエリの追加のパラメーター (`$filter` と `$select` など) を表します 

さらに、要求の一部としてテナントを指定できます (省略可能であり、必須ではありません)。`me` を使用する場合は、テナントを指定しません。一般的な要求の概要については、[概要](overview.md)をご覧ください。
...
メタデータを使用すると、エンティティ、エンティティの種類とセット、複合型、Microsoft Graph REST API の列挙型を参照できます。メタデータとすぐに利用可能なサードパーティ製ツールを使用して、シリアル化されたオブジェクトを作成し、REST API の使用を簡略化するクライアント ライブラリを生成できます。  

リソース URL は、Microsoft Graph エンティティ データ モデルによって決まります。方法は、エンティティ メタデータ スキーマ ($metadata) に概要が示されています。 

パス URL リソース名、クエリ パラメーター、ならびにアクション パラメーターおよびアクション値は、大文字と小文字が区別されません。ただし、割り当てた値、エンティティ ID、その他の base64 でエンコードされた値では大文字と小文字を区別します。

API へのいくつかの基本的なプログラミング パターン呼び出しを、次のセクションに示します。

###セットからメンバーへの移動

ユーザーに関する情報を表示するには、HTTPS GET 要求を使用して、`users` コレクションから識別子によって識別される特定ユーザーの `User` エンティティを取得します。`User` エンティティでは、`id` プロパティと `userPrincipalName` プロパティのどちらかを識別子として使用できます。次の要求例では、ユーザーの ID として `userPrincipalName` の値が使用されています。 
...
同様に、リレーションシップをたどることで関連リソースに移動できます。たとえば、`user => messages` リレーションシップは、Azure AD ノードから Exchange Online ノードへのグラフ走査を可能にします。次の例は、REST API 呼び出しでこれを行う方法を示しています。
...
Microsoft Graph API では、リソースを操作する OData のアクションと関数もサポートされています。たとえば、次の HTTPS POST 要求は、サインインしているユーザー (`me`) に電子メール メッセージを送信させます。
```no-highlight 
